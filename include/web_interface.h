...
<body>
  ...
  <div class="grid">
    ...
    <div class="card">
        ...
        <div class="stat-row"><span>UART Status</span><span id="h_uart">---</span></div>
        ...
        <div style="display:flex; gap:5px; margin-top:10px; flex-wrap: wrap;">
            ...
            <button class="warn" style="flex:1; min-width:80px;" onclick="if(confirm('Really perform factory reset of radar?')) api('radar/factory_reset', {method:'POST'})">Reset MW</button>
        </div>
    </div>
    ...
    <div class="card">
        ...
        <div id="tab0">
            ...
            <div class="stat-row" style="margin-top:10px"><span>Motion Sensitivity (%)</span></div>
            ...
            <div style="display:flex; align-items:center; gap:8px; margin-bottom:10px">
                ...
                <label for="chk_eng" title="Enables detailed gate data (14 zones) and faster communication">Enable Diagnostics</label>
            </div>
        ...
        <div id="tab1" class="hidden">
            ...
            <p style="font-size:0.7rem; color:#666; margin:2px 0 15px 0">
                Alert when someone stands &lt;2m from sensor longer than timeout.
            </p>
            ...
            <div style="display:flex; align-items:center; gap:8px; margin-bottom:5px">
                ...
                <label for="chk_dis_rem">"Still DISARMED" reminder</label>
            </div>
            ...
            <p style="font-size:0.7rem; color:#666; margin:2px 0 15px 0">
                Duration after which the radar reports "unoccupied" with no detection.
            </p>
        ...
        <div id="tab2" class="hidden">
            ...
            <div style="font-size:0.75rem; color:#888; margin-bottom:8px; line-height:1.5">
                <span style="color:#03dac6; font-weight:bold">&#9632; Motion</span> = sensitivity to moving objects &middot;
            ...
            <div style="display:flex; justify-content:space-between; margin-bottom:8px; gap:5px">
                ...
                <button class="sec" style="flex:1; padding:5px; font-size:0.8rem" onclick="setPreset('pet')">Pet</button>
            </div>
        ...
        <div id="tab5" class="hidden">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px">
                ...
                <button onclick="clearEvents()" class="warn" style="width:auto; padding:5px 10px; margin:0">Clear History</button>
            </div>
    ...
    <!-- OTA -->
    <div class="card">
        <div class="stat-row"><span>FW Update</span></div>
...
function updateUI(d) {
    ...
    let st = "IDLE";
    let stColor = "#888";
    if(d.state === "detected") { st = "DETECTED"; stColor = "var(--accent)"; }
    else if(d.state === "hold") { st = "HOLDING"; stColor = "#bb86fc"; }
    ...
    fetch('/api/events').then(r=>r.json()).then(d => {
        ...
        d.forEach(e => {
            ...
            if (u > 1700000000) {
                ...
                timeStr = d.toLocaleString('en-US', {day:'numeric',month:'numeric', hour:'2-digit',minute:'2-digit'});
            } else {
...
function startCalib() {
    if(confirm("Start noise calibration? (60s, do not move in front of sensor during this time)")) {
...
function testTelegram() {
    ...
    .catch(e => showToast("Communication error"));
}
    ...
    zones.forEach((z, i) => {
        ...
        h += `<div style="margin-bottom:5px; background:#222; padding:5px; border-radius:5px; border-left:3px solid ${ZONE_COLORS[ab]||'#444'}">
            <div style="display:flex; gap:5px; margin-bottom:5px">
                ...
                <input type="number" value="${z.min}" id="z_min_${i}" style="flex:1" placeholder="From (cm)">
                <input type="number" value="${z.max}" id="z_max_${i}" style="flex:1" placeholder="To (cm)">
            </div>
            <div style="display:flex; gap:5px; align-items:center">
                ...
                <select id="z_ab_${i}" style="flex:2" title="Zone alarm behavior">
                    ...
                    <option value="2" ${ab==2?'selected':''}>🔕 Ignore</option>
                    <option value="3" ${ab==3?'selected':''}>📡 Ignore static</option>
                </select>
...
function pollLearn() {
    fetch('/api/radar/learn-static').then(r=>r.json()).then(d => {
        ...
        if (!d.active && d.progress === 100) {
            ...
            let txt = `✅ Done! Top gate: ${d.top_gate} (~${d.top_cm}cm), confidence: ${d.confidence}%`;
            if (d.suggest_ready) {
...
function saveGates() {
    ...
    fetch('/api/radar/gates', {
        ...
        else showToast("Gate save error");
    });
}
function setPreset(t) {
    fetch('/api/preset?name=' + t, {method:'POST'}).then(r => {
        if(!r.ok) { showToast("Preset error"); return; }
...
function updateAlarmUI(state) {
    ...
    else if(state === 'triggered') { badge.innerText = '🚨 ALARM'; badge.style.color='red'; btn.innerText='DISARM'; btn.style.background='#3700b3'; }
}
function toggleArm() {
    if(alarmArmed) {
...
function saveAuth() {
    ...
    if(u.length < 4 || p.length < 4) { showToast("Min. 4 characters"); return; }
