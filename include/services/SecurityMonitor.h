...
public:
    ...
    // Enable/Disable toggles (for vacant locations - storage, cabins, server rooms)
    ...
    // Alarm event (atomic JSON for MQTT — single-use)
...
private:
    ...
    // Enable/Disable (for vacant locations - storage, cabins, server rooms)
    bool _antiMaskEnabled = false;   // DEFAULT: OFF - most locations are occasionally empty
