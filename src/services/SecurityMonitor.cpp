...
void SecurityMonitor::processRadarData(uint16_t distance, uint8_t move_energy, uint8_t static_energy) {
    ...
    // Pet Immunity — ignore low energy (both move and static)
    ...
    // For vacant locations (storage, cabins, server rooms) silence may be normal
    ...
    if (newZoneName != _currentZoneName) {
        ...
        DBG("SecMon", "Entered Zone: %s", _currentZoneName.c_str());
    ...
    if (armedQualifies && _armedDebounceCount >= _alarmDebounceFrames) {
        ...
        if (behavior == 2) {
            ...
            // ignore_static_only: fixed reflectors (e.g. metal ladder) generate only static energy without motion
        ...
        // zone/distance/energy preserved from last PENDING event — update uptime
        _pendingEvent.uptime_s = now / 1000; fillISOTime(_pendingEvent.iso_time, sizeof(_pendingEvent.iso_time));
        enqueueAlarmEvent();
...
String SecurityMonitor::formatApproachLog() const {
    ...
    String result = "📍 Approach (" + String(_approachCount) + " records):\n";
